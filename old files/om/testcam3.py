...
# Initialize Picamera2 with higher resolution settings
picam2 = Picamera2()
picam2.start()
...
# Initialize the motor control program (using subprocess)
motor_process = subprocess.Popen(["./motor_control"], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
...
try:
    while True:
        # Capture the frame using Picamera2 (high resolution)
        ...
        # Histogram equalization to improve contrast and illumination
        gray_frame = cv2.equalizeHist(gray_frame)

        # Detect faces using Haar cascade
        faces = face_cascade.detectMultiScale(
            gray_frame,
            scaleFactor=1.1,      # Scaling factor to account for faces at different distances
            minNeighbors=5,       # Minimum number of neighbors to consider a face
            minSize=(30, 30),     # Minimum size of the face to detect
            maxSize=(300, 300)    # Maximum size of the face to detect (avoid false positives from large objects)
        )
        ...
        for i, (x, y, w, h) in enumerate(faces):
            ...
            if i == best_match_index:
                ...
                cv2.putText(frame, "Target", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)  # Red text
                ...
                cv2.putText(frame, "Unknown", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)  # Green text
